...
type Candidate = SuggestionItem & {
  tags:     string[]
  cls:      SuggestionClass
  windows?: string[]  // required when cls === 'time-locked'
}

// ─── Candidate pool ───────────────────────────────────────────────────────────
//
// Presence items (home, away, out, back) are intentionally excluded —
// those are handled by the Tap In bar.
//
// Scoring priority: time fit > space fit > recent usage

const CANDIDATES: Candidate[] = [
  // ── Time-locked: excluded entirely outside their valid time window ─────────
  ...
  // ── Repeatable: may occur multiple times per day; soft recency penalty ─────
  ...
  // ── Contextual: ranked by space + time; no hard time gate ─────────────────
...
function detectSpaceType(name: string): string {
  ...
  if (/team|practice|league|gym|court|dance|volleyball|soccer|hockey|yoga/.test(n)) return 'team'
  return 'generic'
}
...
// Network Information API — experimental, unavailable on iOS Safari
...
// ─── Engine ───────────────────────────────────────────────────────────────────
//
// Scoring weights implement: time fit > space fit > recent usage
//
//   time-locked in window  +20   primary signal — strong time relevance
//   tod match (non-locked) +12   good time signal for repeatable/contextual
//   space match             +8   secondary signal
//   network match           +3   weak ambient signal
//   recency boost      0 – +8   capped so history can't override context
...
export function buildSuggestions(
  ...
  // Recent label frequency — own events weighted 2×, decayed by position
  ...
  // Most recent logged timestamp per label — used for recency penalty
  ...
  for (const c of CANDIDATES) {
    ...
    // Time-locked: hard-exclude outside valid window
    if (c.cls === 'time-locked' && !c.windows!.includes(tod)) continue

    let score = 0

    // 1. Time fit (primary)
    if (c.cls === 'time-locked') {
      score += 20                          // in-window bonus
    } else if (c.tags.includes(tod)) {
      score += 12                          // tod tag match
    }

    // 2. Space fit (secondary)
    if (c.tags.includes(spaceType)) score += 8

    // 3. Network (ambient)
    if (c.tags.includes(network)) score += 3

    // 4. Recent behavior boost — capped so it can't override context signals
    score += Math.min(freq.get(key) ?? 0, 8)

    // 5. Recency penalty for repeatable and time-locked items
    const last = lastLogged.get(key)
    if (last !== undefined) {
      const minAgo = (now - last) / 60_000
      if (c.cls === 'repeatable') {
      ...
      // contextual: no recency penalty — these are set-and-forget entries
    }
  ...
  // Take top 10, cap emoji icons at 2 across the full set
